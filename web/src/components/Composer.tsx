...
export default function Composer({ url, selector: initialSelector, onSuccess, onCancel }: ComposerProps) {
    ...
    const highlightedText = selector?.type === "TextQuoteSelector" ? selector.exact : null;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!text.trim()) {
                await createHighlight({ url, selector: finalSelector, color: "yellow", tags: tagList });
            } else {
                await createAnnotation({ url, text: text.trim(), selector: finalSelector || undefined, tags: tagList });
            }
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-surface-900 dark:text-white">New Annotation</h3>
                {url && <div className="text-xs text-surface-400 dark:text-surface-500 max-w-[200px] truncate">{url}</div>}
            </div>
                    ...
                    {!showQuoteInput ? (
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex justify-end">
                                <button type="button" className="text-xs text-red-500 dark:text-red-400 font-medium" onClick={handleRemoveSelector}>
                                    Remove Quote
                                </button>
                            </div>
                        </div>
            ...
            <textarea
                ...
                placeholder={highlightedText || quoteText ? "Add your comment..." : "Write your annotation..."}
            ...
            <div className="flex items-center justify-between pt-2">
                <span className={text.length > 2900 ? "text-red-500 dark:text-red-400 text-xs font-medium" : "text-surface-400 dark:text-surface-500 text-xs"}>
                    {text.length}/3000
                </span>
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        disabled={loading || (!text.trim() && !highlightedText && !quoteText.trim())}
                    >
                        {loading ? "..." : "Post"}
                    </button>
                </div>
            ...
            {error && <div className="text-red-500 dark:text-red-400 text-sm text-center bg-red-50 dark:bg-red-900/20 py-2 rounded-lg">{error}</div>}
        </form>
    );
}
