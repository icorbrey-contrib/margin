...
const ReplyItem: React.FC<ReplyItemProps> = ({ reply, depth = 0, user, onReply, onDelete, isInline }) => {
        ...
        <div key={reply.uri || reply.id}>
            <div
                className={clsx(
                    ...
                    depth > 0 && "ml-4 pl-3 border-l-2 border-surface-200 dark:border-surface-700"
                )}
            >
                {isInline ? (
                    <>
                        <a href={`/profile/${author.handle}`} className="shrink-0">
                            {getAvatarUrl(author.did, author.avatar) ? (
                                <img
                                    ...
                                    alt=""
                                    className={clsx("rounded-full object-cover bg-surface-200 dark:bg-surface-700", depth > 0 ? "w-6 h-6" : "w-7 h-7")}
                                />
                            ) : (
                                <div className={clsx("rounded-full bg-surface-200 dark:bg-surface-700 flex items-center justify-center text-surface-500 dark:text-surface-400 font-bold", depth > 0 ? "w-6 h-6 text-[10px]" : "w-7 h-7 text-xs")}>
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-baseline gap-2 mb-0.5 flex-wrap">
                                <span className={clsx("font-medium text-surface-900 dark:text-white", depth > 0 ? "text-xs" : "text-sm")}>
                            ...
                            <p className={clsx("text-surface-800 dark:text-surface-200 whitespace-pre-wrap leading-relaxed", depth > 0 ? "text-sm" : "text-sm")}>
                    ...
                    <div className="p-3 bg-white dark:bg-surface-900 rounded-lg ring-1 ring-black/5 dark:ring-white/5">
                        <div className="flex items-center gap-2 mb-2">
                            <a href={`/profile/${author.handle}`} className="shrink-0">
                                {getAvatarUrl(author.did, author.avatar) ? (
                                    <img src={getAvatarUrl(author.did, author.avatar)} alt="" className="w-7 h-7 rounded-full object-cover bg-surface-200 dark:bg-surface-700" />
                                ) : (
                            ...
                            <div className="flex flex-col">
                                <span className="font-medium text-surface-900 dark:text-white text-sm">{author.displayName || author.handle}</span>
                            </div>
                        ...
                        <div className="flex items-center justify-end gap-2 pl-9">
                            <button onClick={() => onReply(reply)} className="text-surface-400 dark:text-surface-500 hover:text-primary-600 dark:hover:text-primary-400 transition-colors p-1">
                                <Reply size={14} />
                            </button>
                            {isReplyOwner && (
                                <button onClick={() => onDelete(reply)} className="text-surface-400 dark:text-surface-500 hover:text-red-500 dark:hover:text-red-400 transition-colors p-1">
                                    <Trash2 size={14} />
                                </button>
                            )}
                        </div>
            ...
            {reply.children && reply.children.length > 0 && (
                <div className="flex flex-col">
                    {reply.children.map((child) => (
                        <ReplyItem key={child.uri || child.id} reply={child} depth={depth + 1} user={user} onReply={onReply} onDelete={onDelete} isInline={isInline} />
                    ))}
                </div>
            )}
        </div>
...
export default function ReplyList({ replies, rootUri, user, onReply, onDelete, isInline = false }: ReplyListProps) {
    if (!replies || replies.length === 0) {
        return (
            <div className="py-8 text-center">
                <p className="text-surface-500 dark:text-surface-400 text-sm">No replies yet</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-1">
            {replyTree.map((reply) => (
                <ReplyItem key={reply.uri || reply.id} reply={reply} depth={0} user={user} onReply={onReply} onDelete={onDelete} isInline={isInline} />
            ))}
        </div>
    );
}
