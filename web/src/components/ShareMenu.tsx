...
import { Copy, ExternalLink, Check, Share2, MoreHorizontal } from "lucide-react";
import { AturiIcon, BlueskyIcon, BlackskyIcon, WitchskyIcon, CatskyIcon, DeerIcon } from "./Icons";
...
export default function ShareMenu({ uri, text, customUrl, handle, type, url }: ShareMenuProps) {
    ...
    const getShareUrl = () => {
        ...
        if (uri.includes("network.cosmik.card")) return `${window.location.origin}/at/${did}/${rkey}`;
        if (handle && type) return `${window.location.origin}/${handle}/${type.toLowerCase()}/${rkey}`;
    ...
    const sembleUrl = (() => {
        ...
        if (uri.includes("network.cosmik.collection")) return `https://semble.so/profile/${userHandle}/collections/${rkey}`;
        if (uri.includes("network.cosmik.card") && url) return `https://semble.so/url?id=${encodeURIComponent(url)}`;
    ...
    const handleShareToFork = (domain: string) => {
        const composeText = text ? `${text.substring(0, 200)}...\n\n${shareUrl}` : shareUrl;
    ...
    const renderMenuItem = (label: string, icon: React.ReactNode, onClick: () => void, isCopied: boolean = false, highlight: boolean = false) => (
        <button
            onClick={onClick}
        ...
        >
            <span className={`flex items-center justify-center w-5 h-5 ${highlight ? "text-primary-600 dark:text-primary-400" : "text-surface-400 dark:text-surface-500 group-hover:text-surface-600 dark:group-hover:text-surface-300"}`}>
                {isCopied ? <Check size={16} className="text-green-600 dark:text-green-400" /> : icon}
            </span>
    ...
    const shareForks = [
        { name: "Bluesky", domain: "bsky.app", icon: <BlueskyIcon size={18} color={BLUESKY_COLOR} /> },
        { name: "Witchsky", domain: "witchsky.app", icon: <WitchskyIcon size={18} /> },
        { name: "Blacksky", domain: "blacksky.community", icon: <BlackskyIcon size={18} /> },
    ...
    return (
        <div className="relative inline-block">
            <button
                ref={buttonRef}
                onClick={toggleMenu}
                className={`flex items-center gap-1.5 px-2 py-1.5 -ml-2 rounded-md transition-colors ${isOpen ? 'bg-surface-100 dark:bg-surface-800 text-surface-900 dark:text-white' : 'text-surface-500 dark:text-surface-400 hover:text-surface-900 dark:hover:text-white hover:bg-surface-50 dark:hover:bg-surface-800'}`}
                title="Share"
            >
                <Share2 size={18} />
            </button>
                    ...
                    <div className="flex flex-col gap-0.5">
                        {isSemble ? (
                            <>
                                ...
                                {renderMenuItem("Open on Semble", <ExternalLink size={16} />, () => window.open(sembleUrl, "_blank"), false, true)}
                                {renderMenuItem("Copy Semble Link", <Copy size={16} />, () => handleCopy(sembleUrl, 'semble'), copied === 'semble')}
                                <div className="h-px bg-surface-100 dark:bg-surface-800 my-1 mx-2" />
                            </>
                        ) : null}

                        {renderMenuItem("Copy Link", <Copy size={16} />, () => handleCopy(shareUrl, 'link'), copied === 'link')}
                        ...
                        {renderMenuItem("Copy Universal Link", <AturiIcon size={16} />, () => handleCopy(uri.replace("at://", "https://aturi.to/"), 'aturi'), copied === 'aturi')}

                        {navigator.share && (
                            renderMenuItem("More Options...", <MoreHorizontal size={16} />, () => {
                                navigator.share({ title: "Margin", text, url: shareUrl }).catch(() => { });
                                setIsOpen(false);
                            })
                        )}
                    </div>
