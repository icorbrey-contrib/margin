...
import { X, Plus, Check, Loader2, ChevronRight, FolderPlus } from 'lucide-react';
import { useStore } from '@nanostores/react';
import { $user } from '../store/auth';
import { getCollections, addCollectionItem, createCollection, type Collection } from '../api/client';
...
export default function AddToCollectionModal({ isOpen, onClose, annotationUri }: AddToCollectionModalProps) {
        ...
        >
            ...
            >
                <div className="p-4 flex justify-between items-center border-b border-surface-100 dark:border-surface-800">
                    <h2 className="text-xl font-display font-bold text-surface-900 dark:text-white">Add to Collection</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="px-6 pb-6 pt-4">
                    {loading ? (
                        <div className="text-center py-10">
                            <Loader2 size={32} className="animate-spin text-primary-600 dark:text-primary-400 mx-auto mb-3" />
                            <p className="text-surface-500 dark:text-surface-400 font-medium">Loading collections...</p>
                        </div>
                    ) : showNewForm ? (
                        ...
                        <div>
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-bold text-surface-900 dark:text-white group-hover:text-primary-700 dark:group-hover:text-primary-400 transition-colors">New Collection</h3>
                                    <span className="text-sm text-surface-500 dark:text-surface-400">Create a new collection</span>
                                </div>
                                <ChevronRight size={20} className="text-surface-300 dark:text-surface-600 group-hover:text-primary-500 dark:group-hover:text-primary-400" />
                            </button>

                            {collections.length === 0 ? (
                                <div className="text-center py-6">
                                    <p className="text-surface-500 dark:text-surface-400">No collections yet</p>
                                </div>
                            ) : (
                                            ...
                                            >
                                                <div className="w-8 h-8 flex items-center justify-center bg-white dark:bg-surface-700 rounded-full shadow-sm text-surface-600 dark:text-surface-300">
                                                    {col.icon ? (
                                                        ...
                                                        <span className="font-bold text-xs">{col.name[0]}</span>
                                                    )}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <h3 className="text-sm font-bold text-surface-900 dark:text-white">{col.name}</h3>
                                                    {col.description && (
                                                        <p className="text-xs text-surface-500 dark:text-surface-400 line-clamp-1">{col.description}</p>
                                                    )}
                                                </div>
                                                {isAdding ? (
                                                    ...
                                                    <Plus size={16} className="text-surface-300 dark:text-surface-500 group-hover:text-surface-600 dark:group-hover:text-surface-300" />
                                                )}
                                            </button>
                                        );
                                    })}
