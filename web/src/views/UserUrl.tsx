...
import { PenTool, Highlighter, Search, AlertTriangle, ExternalLink } from 'lucide-react';
...
export default function UserUrlPage() {
    ...
    const displayHandle = profile?.handle || (handle?.startsWith("did:") ? null : handle);
    const avatarUrl = getAvatarUrl(profile?.did, profile?.avatar);

    const getInitial = () => {
        return (displayName || displayHandle || "??")?.substring(0, 2).toUpperCase();
    };
        ...
        if (activeTab === "annotations" && annotations.length === 0) {
            return (
                <div className="flex flex-col items-center justify-center p-12 text-center bg-surface-50 border border-dashed border-surface-200 rounded-2xl">
                    ...
                    <h3 className="text-lg font-medium text-surface-600">No annotations</h3>
                </div>
            );
        }
        ...
        if (activeTab === "highlights" && highlights.length === 0) {
            return (
                <div className="flex flex-col items-center justify-center p-12 text-center bg-surface-50 border border-dashed border-surface-200 rounded-2xl">
                    ...
                    <h3 className="text-lg font-medium text-surface-600">No highlights</h3>
                </div>
            );
        }
    ...
    if (!targetUrl) {
        return (
            <div className="max-w-2xl mx-auto py-20 text-center">
                ...
                <h3 className="text-xl font-bold text-surface-900 mb-2">No URL specified</h3>
                <p className="text-surface-500">Please provide a URL to view annotations.</p>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-3xl mx-auto pb-20">
            <header className="flex items-center gap-6 mb-8 p-6 bg-white rounded-2xl border border-surface-200 shadow-sm">
                ...
                >
                    {avatarUrl ? (
                        <img src={avatarUrl} alt={displayName} className="w-20 h-20 rounded-full object-cover border-4 border-surface-50" />
                    ) : (
                ...
                <div className="flex-1">
                    <h1 className="text-2xl font-bold text-surface-900 mb-1">{displayName}</h1>
                    {displayHandle && (
                        <a
                            href={bskyProfileUrl}
                            target="_blank"
            ...
            <div className="mb-8 p-4 bg-surface-50 border border-surface-200 rounded-xl flex flex-col sm:flex-row sm:items-center gap-4">
                <span className="text-sm font-semibold text-surface-500 uppercase tracking-wide">Annotations on:</span>
                <a
            ...
            {!loading && !error && totalItems === 0 && (
                <div className="text-center py-16 bg-surface-50 rounded-2xl border border-dashed border-surface-200">
                    ...
                    <h3 className="text-lg font-bold text-surface-900 mb-1">No items found</h3>
                ...
                <div className="animate-fade-in">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                        ...
                        <div className="flex bg-surface-100 p-1 rounded-xl self-start md:self-auto">
                            <button
                                className={clsx("px-4 py-1.5 rounded-lg text-sm font-medium transition-all", activeTab === 'all' ? "bg-white text-surface-900 shadow-sm" : "text-surface-500 hover:text-surface-700")}
                                onClick={() => setActiveTab('all')}
                            >
                                All ({totalItems})
                            </button>
                            <button
                                className={clsx("px-4 py-1.5 rounded-lg text-sm font-medium transition-all", activeTab === 'annotations' ? "bg-white text-surface-900 shadow-sm" : "text-surface-500 hover:text-surface-700")}
                                onClick={() => setActiveTab('annotations')}
                            >
                                Annotations ({annotations.length})
                            </button>
                            <button
                                className={clsx("px-4 py-1.5 rounded-lg text-sm font-medium transition-all", activeTab === 'highlights' ? "bg-white text-surface-900 shadow-sm" : "text-surface-500 hover:text-surface-700")}
