...
import { getCollection, getCollectionItems, deleteCollection, removeCollectionItem, resolveHandle } from '../api/client';
...
export default function CollectionDetail({ handle, rkey, uri }: CollectionDetailProps) {
    ...
    const handleRemoveItem = async (item: AnnotationItem) => {
        ...
        if (success) {
            setItems(prev => prev.filter(i => i.collectionItemUri !== item.collectionItemUri));
        }
    };
    ...
    if (error || !collection) {
        return <div className="text-center py-20 text-red-500 dark:text-red-400">{error || "Collection not found"}</div>;
    }
    ...
    return (
        <div className="animate-fade-in max-w-2xl mx-auto">
            <a href="/collections" className="inline-flex items-center gap-1.5 text-sm font-medium text-surface-500 dark:text-surface-400 hover:text-surface-900 dark:hover:text-white mb-4 transition-colors">
                <ArrowLeft size={16} />
                Collections
            </a>
            ...
            <div className="bg-white dark:bg-surface-900 rounded-xl p-4 ring-1 ring-black/5 dark:ring-white/5 mb-4">
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        <h1 className="text-xl font-bold text-surface-900 dark:text-white truncate">{collection.name}</h1>
                        {collection.description && (
                            <p className="text-surface-600 dark:text-surface-300 text-sm mt-1">{collection.description}</p>
                        )}
                        <div className="flex items-center gap-2 mt-2 text-xs text-surface-500 dark:text-surface-400">
                            ...
                            <span>by {collection.creator.displayName || collection.creator.handle}</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-1">
                        ...
                        />
                        {isOwner && (
                            <button onClick={handleDelete} className="p-2 text-surface-400 dark:text-surface-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                                <Trash2 size={18} />
                            </button>
                        )}
                    </div>
            ...
            <div className="space-y-2">
                {items.length === 0 ? (
                    <div className="text-center py-12 text-surface-500 dark:text-surface-400 bg-surface-50 dark:bg-surface-800/50 rounded-xl border border-dashed border-surface-200 dark:border-surface-700">
                        <Plus size={28} className="mx-auto mb-2 text-surface-300 dark:text-surface-600" />
